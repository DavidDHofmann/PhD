...
\begin{figure}[h]
  \begin{center}
    \begin{tikzpicture}
        ...
        \includegraphics[width=0.9\textwidth]{99_StudyArea.pdf}
        };
...
\Cref{Trajectories} in \Cref{Appendix:GPSData}). In our analysis, we did not
  ...
  \begin{center}
    \resizebox{\textwidth}{!}{
      ...
      \parbox[]{2cm}{\centering Coalition ID} &
        \parbox[]{2cm}{\centering Sex} &
          \parbox[]{2cm}{\centering \ Pack \\ Affiliation} &
            \parbox[]{2cm}{\centering \# Fixes \\ Total} &
              \parbox[]{3cm}{\centering \# Fixes \\ During Dispersal} &
                \parbox[]{2cm}{\centering \# Days \\ Dispersing} &
                  \parbox[]{4cm}{\centering Euclidean Dispersal\\ Distance (in km)} &
                    \parbox[]{4cm}{\centering Cumulative Dispersal \\ Distance (in km)} \\
      \midrule
...
different, we calculated a threshold (\(t\)) using \Cref{EQ1}:
...
\begin{table}[h]
  ...
  model. Weights were calculated using: \(\text{AIC-Weight}_i = \frac{e^{-0.5}
  \Delta \text{AIC}_i} {\sum_{i = 1}^n e^{-0.5} \Delta \text{AIC}_i} \). The
  most parsimonious model outperformed all other models (\(\Delta AIC > \) 2)
  and received a weight of one. W = Water, DTW = DistanceToWater, S =
  Shrubs/Grassland, T = Trees, P = Protected, HB5 = HumansBuff5000, RC =
  RoadCrossing, DTR = DistanceToRoads.}
  \label{ModelAICs}
  \begin{center}
    \resizebox{\textwidth}{!}{
      \begin{tabular}{lllll}
      \toprule
        ...
        \midrule
        cos(ta) + log(sl) + W + S + DTW + HB5 + T & 90099.62 & 0.00 & 1.00 & -45034.81 \\
        cos(ta) + log(sl) + W + S + DTW + HB5 + T + DTR & 90101.67 & 2.04 & 0.00 & -45033.83 \\
        cos(ta) + log(sl) + W + S + DTW + HB5 + T + P & 90103.56 & 3.93 & 0.00 & -45034.78 \\
        cos(ta) + log(sl) + W + S + DTW + HB5 & 90104.31 & 4.69 & 0.00 & -45039.16 \\
        cos(ta) + log(sl) + W + S + DTW + T & 90104.98 & 5.36 & 0.00 & -45039.49 \\
        cos(ta) + log(sl) + W + S + DTW + HB5 + T + DTR + P & 90105.67 & 6.04 & 0.00 & -45033.83 \\
        cos(ta) + log(sl) + W + S + DTW + HB5 + T + RC & 90105.84 & 6.22 & 0.00 & -45033.92 \\
        cos(ta) + log(sl) + W + S + DTW + HB5 + DTR & 90106.27 & 6.65 & 0.00 & -45038.14 \\
        cos(ta) + log(sl) + W + S + DTW + HB5 + T + DTR + RC & 90107.87 & 8.25 & 0.00 & -45032.93 \\
        cos(ta) + log(sl) + W + S + DTW + HB5 + P & 90108.23 & 8.61 & 0.00 & -45039.12 \\
        cos(ta) + log(sl) + W + S + DTW & 90110.11 & 10.49 & 0.00 & -45044.06 \\
        cos(ta) + log(sl) + W + S + DTW + HB5 + RC & 90110.66 & 11.03 & 0.00 & -45038.33 \\
        cos(ta) + log(sl) + W + S + DTW + DTR & 90111.70 & 12.07 & 0.00 & -45042.85 \\
