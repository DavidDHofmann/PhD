...
selection analysis to estimate relative selection strenghts\todo{as per reviewer
1} of dispersers and to create a permeability surface for the entire KAZA-TFCA.
We compared landscape permeability across different regions within the KAZA-TFCA
as well as outside its boundaries. Lastly, we calculated least-cost paths and
corridors to verify that major movement routes were adequately encompassed
...
degrees of human activities across regions, as well as to the presence of
rivers, swamps or open water. The relation between other landscape features and
landscape permeability was less evident.\todo{Editor indicated that the
...
especially where transboundary dispersal occurs.\todo{According to suggestions
by reviewer 2}
\end{abstract}
...
it according to his suggestion.} importance for conservation management
...
species' relative selection strengths\todo{reviewer 1}, which can be quantified
using a suite of selection functions \citep{Zeller.2012}. Specifically, relative
selection strenghts \todo{reviewer 1} are estimated by comparing spatial
...
\todo{Reviewer 1 pointed out correctly that we should talk about available
locations, rather than randomly selected locations} the animal
...
protected species.\todo{Reviewer 2 desired to change from: this introduces - to:
this can introduce}
...
estimated the relative selection strengths\todo{reviewer 1} towards or agains
...
Behr.2020}\todo{We originally only pointed to Behr and did not include the
criteria. The editor desired to include those}. Individuals were immobilized
...
triggered by a slowly decomposing piece of cloth.\todo{Editor desired more
information on GPS collar} During dispersal, GPS collars were programmed to
...
roads.\todo{Reviewer 2 found the description of the covariates unclear} For each
...
investigate dispersers' relative selection strentgh \todo{Reviewer 1 suggested
to use relative selection strength, rather than preference} for the
...
selection strength \todo{As per reviewer 1} for these covariates (\(\beta_1,
\beta_2, ..., \beta_n\)). To estimate relative selection strength \todo{As per
reviewer 1} (i.e. the \(\beta\)'s) for each covariate, we used mixed effects
...
\textit{log(sl)} (\Cref{PermeabilityResults}a). The positive coefficient for
...
S7).\todo{Reviewer 1 desired more information on the random effects}
...
distane to water was insignificant.\todo{Reviewer 1 suggested to add this} Given
...
populations.\todo{Editor desired that we discuss the applicability of our
ecosystem to other ecosystems.}
...
\todo{Reviewer 1 pointed out that the permeability surface likely overestimates
permeability in areas far from suitable habitat. Additionally, reviewer 2
mentioned that our dataset by far suffices for simulations and we should not
argue with sample size.}
