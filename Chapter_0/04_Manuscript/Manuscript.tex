...
rivers, swamps or open water. The relation between permeability and other
landscape features was less evident.\todo{Editor indicated that the
relationships are correlative and not necessarily causal.}
...
connectivity, especially where transboundary dispersal occurs.\todo{According to
...
\todo{Reviewer one pointed out an awkward use of the word utmost. I have changed
it according to his suggestion.} importance \citep{Doerr.2011, Rudnick.2012},
...
\citep{Taylor.2006, Fattebert.2015}.\todo{As per reviewer 2} Identification of
...
based on species' relative selection strengths \citep{Avgar.2017}\todo{reviewer
1}, which can be quantified using a suite of selection functions
\citep{Zeller.2012}. Specifically, selection strengths \todo{reviewer 1} are
...
avilable to the animal \todo{Reviewer 1 pointed out correctly that we should
talk about available locations, rather than randomly selected locations}
...
coincide \citep{Fattebert.2015},\todo{second reviewer pointed out that space use
can be alike} it appears that relocation data collected on dispersing
...
functional connectivity for their protected species.\todo{Reviewer 2 desired to
change from: this introduces - to: this can introduce}
...
spatial scales \citep{Elliot.2014, Tshipa.2017}.\todo{as per editor} Thus, how
...
estimated relative selection strengths\todo{reviewer 1} towards environmental
...
Behr.2020}\todo{The editor desired to include criteria}. Selected individuals
...
cloth.\todo{Editor desired more information on GPS collar} During dispersal, GPS
...
the squared \todo{Reviewer 1 pointed out correctly that we need to talk about
the \textit{squared} euclidean distance} Euclidean distance of a relocation to a
...
To investigate relative selection strengths \todo{As per reviewer 1} of
dispersing wild dogs, we used a set of geo-referenced covariates
(\Cref{Covariates}) that we aggregated in the categories \textit{land cover},
...
distance to roads.\todo{Reviewer 2 found the description of the covariates
unclear} We prepared all covariates as spatial raster layers from freely
...
investigate dispersers' relative selection strentghs towards \todo{Reviewer 1
suggested to use relative selection strength, rather than preference} the
...
selection strengths \todo{As per reviewer 1} towards these covariates
(\(\beta_1, \beta_2, ..., \beta_n\)). To estimate relative selection strengths
\todo{As per reviewer 1} (i.e. the \(\beta\)'s) for each covariate, we used
...
\citealp{Burnham.2002}) for all other covariates.\todo{Reviewer 1 asked for more
details in the random effects. I therefore decided to update this section too}
The inclusion of movement metrics served to reduce biases in estimated habitat
selection coefficients \todo{as per revierwer 1} that may have arisen due to
...
in under 9 days.\todo{Editor desired a quick summary of the dispersal data.}
...
\textit{sl}, and \textit{log(sl)} (\Cref{PermeabilityResults}a). The positive
...
too little directionality. \todo{Reviewer 1 nicely explained that the
coefficients for log(sl) and cos(ta) imply that the initial distributions were
selection-biased} With respect to environmental covariates, dispersers avoided
...
\leq 0.22\), Figure S7).\todo{Reviewer 1 desired more information on the random
effects}
...
choice.\todo{According to reviewer 1 we should point out that our model compares
available vs. unavailable} This conforms to findings on dispersing wild dogs
...
understanding of the impact of humans on dispersal behavior.\todo{As per
reviewer 2} Nevertheless, successful conservation of this species relies on
...
insignificant.\todo{Reviewer 1 suggested to add this} Given the influence that
...
dispersal.\todo{Editor desired info on applicability}
...
to disperse \citep{Fattebert.2015, Hauenstein.2019}.\todo{As per reviewer 2} The
...
by the KAZA-TFCA. \todo{Reviewer 1 pointed out that the permeability surface
likely overestimates permeability in areas far from suitable habitat.
Additionally, reviewer 2 mentioned that our dataset by far suffices for
simulations and we should not argue with sample size.}

Our work shows how dispersal data of a highly mobile species can be used to
\todo{to instead of t, pointed out by the editor and reviewer 1} identify
...
(\url{https://github.com/DavidDHofmann}).\todo{As per editor}
