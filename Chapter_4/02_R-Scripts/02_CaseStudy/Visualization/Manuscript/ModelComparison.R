...
tbl$Coefficient[seq(2, nrow(tbl), 2)] <- ""

# Add AIC and number of random steps
nsteps <- c("Steps", sapply(dat$Model, function(x) {
  format(summary(x)$nevent, big.mark = ",")
}))
aics <- c("AIC", "-", format(round(dat$AIC)[2:3], big.mark = ","))
tbl <- rbind(tbl, nsteps, aics)

# Make a nice table and write it to file
kbl(tbl, booktabs = T, format = "latex", escape = T, align = "lccc", linesep = "") %>%
  row_spec(row = seq(2, nrow(tbl) - 2, by = 2), font_size = 8, italic = T) %>%
  row_spec(row = nrow(tbl) - 2, hline_after = T) %>%
  writeLines("04_Manuscript/99_CaseStudyTable.tex")
...
# Store it
ggsave("04_Manuscript/99_CaseStudy.png"
  , plot   = p
  , width  = 10
  , height = 6.5
  , scale  = 0.8
