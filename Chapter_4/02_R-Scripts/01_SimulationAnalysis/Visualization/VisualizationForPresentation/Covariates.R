...
p1 <- ggplot(dat_cov, aes(x = x, y = y, fill = value)) +
    ...
    geom_path(data = dat_mov, inherit.aes = F, aes(x = x, y = y), linewidth = 0.2, col = "black") +
    ...
    theme(
      ...
      , strip.background = element_rect(fill  = "gray10", color = "transparent")
...
# Store to file
ggsave("05_Presentation/99_Covariates.png"
  , plot   = p1
  , width  = 6
  , height = 2.5
  , bg     = "transparent"
...
ggsave("05_Presentation/99_CovariatesAppendix.png"
  , plot   = p2
  , width  = 6
  , height = 5
  , bg     = "transparent"
