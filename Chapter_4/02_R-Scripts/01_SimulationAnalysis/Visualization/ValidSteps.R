...
# Visualize
p <- ggplot(all, aes(x = Missingness, y = Steps, col = as.factor(Forgiveness), fill = as.factor(Forgiveness), linetype = as.factor(Forgiveness), ymin = LCI, ymax = UCI)) +
  geom_ribbon(alpha = 0.2, linewidth = 0) +
  geom_line(linewidth = 0.4) +
  geom_vline(xintercept = 1 - 0.78, lty = 2, color = "gray30") +
