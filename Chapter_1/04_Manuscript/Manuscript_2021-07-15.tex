...
movement corridors that promote connectivity has become a task of extraordinary
importance for conservation authorities.

% Quantifying connectivity
During the past two decades, ecologists have largely relied on graph-based
methods, such as least-cost analysis or circuit theory, to investigate
connectivity. Despite their popularity, both methods make several restrictive
assumptions that limit their usefulness in reality. Least-cost analysis, for
instance, assumes that animals move towards a pre-conceived endpoint and choose
a cost-minimizing route accordingly. Circuit theory, in contrast, presumes that
animals move according to a random walk without any directional persistence. To
overcome these shortcomings, dispersal simulations from individual-based
movement models have been proposed, yet a unified framework to simulate
dispersal and assess connectivity is lacking.

% What we do
Here, we examine the use of integrated step selection analysis as a simple but
powerful framework for fitting individual-based movement models and simulating
dispersal. For this, we analyse data of 16 dispersing wild dogs (\textit{Lycaon
pictus}) and parametrize a fully mechanistic movement model describing how
dispersers move through the available landscape. We then employ the model to
simulate 80'000 dispersing wild dogs moving across the world's largest
transboundary conservation area. Finally, we use the simulated trajectories to
derive three complementary connectivity maps, each focusing on a different
...
% Relevance
We conclude that simulations from integrated step selection functions indeed
offer a powerful alternative to traditional connectivity modeling techniques and
requires fewer unrealistic assumptions about dispersal. Nevertheless, some
non-trivial modeling decisions such as the number and duration of simulated
dispersal events need to be considered.
