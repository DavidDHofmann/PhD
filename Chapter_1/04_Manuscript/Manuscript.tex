...
estimation of connectivity and identification and preservation of dispersal
corridors has become a task of extraordinary importance for conservation
...
applied. Yet, due to the almost infinite number of non-trivial decisions a
...
model describing how dispersing wild dogs move through the landscape. Based on
this model, we simulate 80'000 dispersers moving across the extent of the
KAZA-TFCA. We then generate a set of maps, each focused on a different aspect of
connectivity within the KAZA-TFCA. Finally, we discuss the benefits and pitfalls
of such a simulation-based approach and highlight potential improvements to be
made in the future.
...
dispersing wild dogs (\Cref{GraphicalAbstract}). We then employ the model to
...
\Cref{AreasReached}, but it is worth pointing out that the figure is only
...
\section{Discussion} % NEEDS TO BE ENRICHED WITH MORE CITATIONS
