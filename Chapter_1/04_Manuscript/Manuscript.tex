...
assumptions can be relaxed by explicitly simulating dispersal trajectories
across the landscape, a unified approach for such simulations is lacking.

Here, we present a simple three-step approach to simulate dispersal and to
assess connectivity using empirical GPS movement data and a set of habitat
covariates. In step one, we use integrated step-selection functions to fit a
...
links between habitat patches. As a case study, we apply the three-step
approach to GPS data collected on dispersing African wild dogs (\textit{Lycaon
pictus}) inhabiting the Kavango-Zambezi Transfrontier Conservation Area
(KAZA-TFCA) in southern Africa.

Using step-selection functions we successfully parametrized a detailed dispersal
model that described dispersing individuals habitat and movement preferences, as
well as potential interactions among the two. The model substantially
outperformed a model that omitted such interactions and enabled us to simulate
80,000 dispersal trajectories across the study area. By explicitly simulating
individuals, this approach not only requires fewer unrealistic assumptions about
dispersal, but also permits the calculation of a rich suite of connectivity
metrics that together provide a comprehensive view on landscape connectivity.
The three derived connectivity maps revealed several dispersal hotspots and
...
techniques. The approach is therefore useful for a variety of applications in
...
subjective modeling decisions, thus rendering among-system comparisons
difficult.
...
a map boundary, suggesting that we successfully mitigated biases from boundary
