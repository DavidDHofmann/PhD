...
################################################################################
# First of all, we need to make all steps larger than zero. Only if all steps
# are greater than zero we can fit a gamma distribution. So let's add one meter
# to all steps that are currently zero (only four steps)
...
# Let's also create a column indicating these fixes
tracks$InactiveTime <- index

# Create a corrected dt
tracks$dtcorr_ <- tracks$dt_
tracks$dtcorr_[tracks$InactiveTime] <- tracks$dt_[tracks$InactiveTime] / 2
...
# Let's visualize the fit (note that rate = 1 / scale, i.e. scale = 1 / rate)
...
randomSteps <- pbmclapply(
  ...
  # # In case we're using TiSSF, we need to calculate a normalized step length
  # # (normalized to 4 hours). Again, remember that fixes taken during the
  # # "inactive" time, need to be halved.
  # if (method == "TiSSF"){
  #   if (tracks$InactiveTime[x]){
  #     sl_new <- sl_new * tracks$dt_[x] / 8
  #   } else {
  #     sl_new <- sl_new * tracks$dt_[x] / 4
  #   }
  # }
