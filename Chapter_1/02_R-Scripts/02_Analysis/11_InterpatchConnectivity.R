...
# SpatialPoints for first location of each trajectory
first <- sims %>%
...
sims <- left_join(sims, first@data[, c("TrackID", "SourceArea")], by = "TrackID")
...
# edge between A and B and between A and C, but not between B to C.
...
# parks. That is, if a trajectory moves from A through B to C, we generate an
# edge between A and B and between B and C, but not between A to C.
