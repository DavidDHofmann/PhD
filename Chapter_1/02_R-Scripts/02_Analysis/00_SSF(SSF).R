...
# We're going to convert the fixes to steps, yet we want to indicate a new burst
# if a step takes longer than 4.25 hours (8.25 hours between 07:00 and 15:00).
# Let's thus calculate the timelags.
data <- data %>%
  group_by(DogName) %>%
...
# Use the "inactive" indicator to calculate corrected step times (i.e. assuming
# that 8 hours correspond to 4 hours during the time of inactivity)
...
# that the step speed is almost identical to the (normalized) step length under
# iSSF. Yet under TiSSF the step lengths are not comparable because of irregular
# sampling.
...
randomSteps <- pbmclapply(
  ...
  # Draw random step lengths with the fitted parameters. Note that I'm going to
  # adjust the scale parameter so that the distribution matches the (corrected)
  # step duration
