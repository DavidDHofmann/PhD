...
library(scales)       # To specify out of bound values
...
p1 <- ggplot(dat, aes(x = ta_, y = prob, group = sl_, color = sl_)) +
  geom_line(size = 1) +
  dark_theme_classic() +
  coord_capped_cart(
      left   = "both"
...
# Visualize using x-y plot
p2 <- ggplot(dat, aes(x = ta_, y = prob, group = SqrtDistanceToWater, color = SqrtDistanceToWater)) +
  geom_line(size = 1) +
  dark_theme_classic() +
  coord_capped_cart(
      left   = "both"
...
# Visualize using x-y plot
p3 <- ggplot(dat, aes(x = ta_, y = prob, group = HumansBuff5000, color = HumansBuff5000)) +
  geom_line(size = 1) +
  dark_theme_classic() +
  coord_capped_cart(
      left   = "both"
...
p_mov <- ggarrange(p1, p2, p3, p4, p5, p6, p7, p8
  , ncol   = 4
  , nrow   = 2
  , labels = paste0("a", 1:8)
  , vjust  = -0.4
...
# Add to plots
p_mov <- annotate_figure(p_mov, top = text_grob(title_mov, size = 15, col = "white"))
p_hab <- annotate_figure(p_hab, top = text_grob(title_hab, size = 15, col = "white"))

# # Put plots together
p <- ggarrange(p_mov, p_hab, nrow = 2)

# Store them
ggsave("MovementModelInterpretation.png", plot = p, scale = 1.7, bg = "transparent")
