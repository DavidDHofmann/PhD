...
p1 <- ggplot() +
  ...
  geom_sf(data = check_psf, size = 0.2, alpha = 0.2, col = "black") +
...
# Plot everything
p5 <- ggplot() +
  geom_raster(data = r_sf, aes(x = x, y = y, fill = layer)) +
  geom_sf(data = buffer_sf, col = "white", fill = NA, lty = 2) +
  geom_sf(data = steps_sf[2:nrow(steps_sf), ], col = "gray80") +
  geom_sf(data = steps_sf[1, ], col = "red") +
  geom_point(data = from, aes(x = x, y = y)) +
  scale_fill_manual(values = c(NA, "orange"), na.value = "transparent") +
  dark_theme_void() +
  coord_sf(
      xlim = c(-20, 220)
    , ylim = c(-20, 120)
...
p6 <- ggplot() +
  ...
  geom_sf(data = pols_sf, col = "white", fill = "orange", alpha = 0.3) +
  geom_sf(data = buffer_sf, lty = 2, col = "white", fill = NA) +
  dark_theme_void() +
  coord_sf(
      xlim = c(-20, 220)
    , ylim = c(-20, 120)
...
#### Combine Plots
################################################################################
# Add plot titles and remove backgrounds
p2 <- p2 +
...
# Store them
ggsave("06_Presentation/ModelingDecisions.png"
  , plot   = p10
  , height = 6
  , width  = 10
  , bg     = "transparent"
