...
# dates.
pot_dates <- lapply(2000:2022, function(x) {
...
# flood %>%
#   as.data.frame(xy = T) %>%
#   pivot_longer(cols = 3:(n + 2), names_to = "Date", values_to = "Value") %>%
...
readr::write_rds(session, file = "02_R-Scripts/99_SessionInformation/01_DataCleaning/04_Floodmaps.rds")
cat("Done :)\n")
