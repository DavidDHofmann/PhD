...
library(fitdistrplus)   # To fit time for dispersal durations
...
#### Load Required Data
################################################################################
# Load the calibrated movement model, scaling parameters, fitted gamma
# distribution for step-lengths
mod     <- read_rds("03_Data/03_Results/MovementModels.rds")
scaling <- read_rds("03_Data/03_Results/Scaling.rds")
sl_dist <- read_rds("03_Data/03_Results/StepLengthDistribution.rds")

# Load lookup table and associated covariate layers, as well as the areas from
# which we're going to simulate dispersal
areas <- vect("03_Data/02_CleanData/Sources.gpkg")
look  <- read_rds("03_Data/02_CleanData/00_SeasonalAggregates/LookupTable.rds")
covs  <- read_rds("03_Data/02_CleanData/00_SeasonalAggregates/Covariates.rds") %>%
  mutate(Raster = map(Filename, rast))

# Study Design

# Test it
generateLandscape("2000-05-13 07:00:00", covariates = covs, lookup = look)
...
# trees <- rast("03_Data/02_CleanData/Trees.tif")
...
#
# # Also load the associated dates!
# covdates <- read_rds("03_Data/02_CleanData/00_SeasonalAggregates/Timestamps.rds")
#
...
disperse <- function(
    ...
    , formula
    , betas
    , shape             = NULL    # Step Length Distribution
    , scale
    ...
    , stop                = F
  ) {    # Should the simulation stop at a boundary?
