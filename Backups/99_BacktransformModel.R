...
# Make sure the transformations are correct
summary(dat$sl_)
summary(dat$sl_s)
summary(dat$log_sl_)
summary(dat$log_sl_s)
summary(dat$cos_ta_)
summary(dat$cos_ta_s)
...
cbind(
    Scaled          = summary(mod2)$coefficients$cond[, 1]
  , Unscaled        = summary(mod1)$coefficients$cond[, 1]
...
# basically the model we would have liked to fit, yet with step lengths in
# meters.
mod1 <- glmm_clogit(case_ ~
  + cos_ta_
  + sl_
...
write_rds(mod1, "test1.rds")
write_rds(mod2, "test2.rds")
mod1 <- read_rds("test1.rds")
mod2 <- read_rds("test2.rds")
...
# Compare covariates (also to the scaled model on meter scale)
cbind(
    Scaled          = summary(mod2)$coefficients$cond[, 1]
  , Unscaled        = summary(mod1)$coefficients$cond[, 1]
  , Backtransformed = back
...
# Let's store the full model with step lengths in kilometers
write_rds(mod1, "03_Data/03_Results/99_MovementModelKilometers.rds")

################################################################################
#### Backtransform Full Model
################################################################################
# Transform step lengths to meters again
dat$sl_ <- dat$sl_ * 1000
...
# Reload the best movement model
models <- read_rds("03_Data/03_Results/99_MovementModel.rds")
best <- models$Model[[1]]
summary(best)

# Compare the model coefficients to our model in kilometers
cbind(
    ModelKilometers       = summary(mod1)$coefficients$cond[, 1]
  , ModelKilometersScaled = summary(mod2)$coefficients$cond[, 1]
  , ModelOriginal         = summary(best)$coefficients$cond[, 1]
  , ModelOriginalRefit    = summary(mod3)$coefficients$cond[, 1]
)
...
back <- backTransform(mod3, variables = variables, means = means, sds = sds)
...
cbind(
    Backtransformed     = back
  , OriginalKilometers  = summary(mod1)$coefficients$cond[, 1]
)
