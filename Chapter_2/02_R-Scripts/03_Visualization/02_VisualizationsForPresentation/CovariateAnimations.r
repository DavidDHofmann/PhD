...
prec <- prec[[dates > "2019-01-01" & dates < "2019-01-10"]]

# Prepare frames
cat("Preparing plots for precipitation data...\n")
plot_names <- paste0(tempfile(), "_", 1:nlyr(prec), ".png")
invisible(pbmclapply(1:nlyr(prec), ignore.interactive = T, mc.cores = detectCores() - 1, function(x) {
  ...
  p <- ggplot(map, aes(x = x, y = y, fill = value)) +
    ...
    scale_fill_gradientn(
        ...
        colors = rev(brewer.pal(11, name = "RdYlBu"))
      , limits = c(0, 10)
      , oob    = squish
...
# Animate the plots
cat("Preparing animation for precipitation data...\n")
