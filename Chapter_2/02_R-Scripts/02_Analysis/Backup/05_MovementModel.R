...
# Replace step length with step speed
dat <- dat %>%
  mutate(
      sl_    = speed_
    , speed_ = NULL
...
dat <- dat %>% select(c(
    ...
    , Trees
    , Shrubs
    , HumansBuff5000_FB = Facebook_HumanInfluenceBuffer_5000
    , HumansBuff5000_WP = Worldpop_HumanInfluenceBuffer_5000
    , DistanceToHumans_FB = Facebook_DistanceToHumans
    , DistanceToHumans_WP = Worldpop_DistanceToHumans
))
...
dat <- dat %>% mutate(
    ...
    , SqrtDistanceToHumans_FB = sqrt(DistanceToHumans_FB)
    , SqrtDistanceToHumans_WP = sqrt(DistanceToHumans_WP)
    , DistanceToWater         = NULL
    , DistanceToHumans_FB     = NULL
    , DistanceToHumans_WP     = NULL
)
  ...
  mutate(data = map(data, function(x){
    x %>%
      transform(
        ...
        , Trees               = scale(Trees)
        , Shrubs              = scale(Shrubs)
        , HumansBuff5000_FB   = scale(HumansBuff5000_FB)
        , HumansBuff5000_WP   = scale(HumansBuff5000_WP)
        , SqrtDistanceToHumans_FB   = scale(SqrtDistanceToHumans_FB)
        , SqrtDistanceToHumans_WP   = scale(SqrtDistanceToHumans_WP)
      )
}))
...
mod <- glmm_clogit(writeForm(c("Water", "SqrtDistanceToWater", "Trees", "Shrubs", "SqrtDistanceToHumans_WP")), data = dat$data[[2]])
summary(mod)
################################################################################
...
# if they all remain in the full model. Note that we're using the tiSSF dataset
# for this.
covars <- c(
    "Water"
  , "SqrtDistanceToWater"
...
# write_rds(models, "03_Data/03_Results/99_BaseModel_FB.rds")
# write_rds(models, "03_Data/03_Results/99_BaseModel_WP.rds")
models <- read_rds("03_Data/03_Results/99_BaseModel_WP.rds")
...
# Plot the model coefficients of the best model
showCoeffs(getCoeffs(models$Model[[1]])[-1, ], xlim = c(-1, 1))
...
# Store the table
write_rds(table, "03_Data/03_Results/99_MovementModelAICs_FB.rds")
write_rds(table, "03_Data/03_Results/99_MovementModelAICs_WP.rds")
...
# Store the result for later
write_rds(models, "03_Data/03_Results/99_MovementModel_FB.rds")
write_rds(models, "03_Data/03_Results/99_MovementModel_WP.rds")
