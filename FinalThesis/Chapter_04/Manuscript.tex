...
Gabriele Cozzi \orcid{0000-0002-1744-1940}, and
John Fieberg \orcid{0000-0002-3180-7021}
...
%points that are needed to approximate the integral in \ref{EQ3} and, even
...
(\Cref{SimulatedCovariates}). The third layer, \textsf{Forest} (categorical),
...
\begin{enumerate}
  ...
  across all $\Delta t$ (\Cref{DistributionParameters}), we resampled data to
...
(\Cref{ModelComparison}). Here, we report on results for a landscape with
autocorrelation of 20, while either holding constant missingness at a
conservative 20\% (\Cref{ModelComparisonForgiveness}) or the forgiveness level
at two (\Cref{ModelComparisonMissingness}) (results for all other combinations
are summarized in \Cref{ModelComparison}). The \textit{imputation} approach
...
(\Cref{ModelComparisonForgiveness}). Note, the \textit{imputation} approach
...
\textsf{DistanceToWater} and \textsf{Trees} (\Cref{CaseStudyCovariates}).
...
250 m and merged them into a single raster-stack (\Cref{CaseStudyCovariates}).
...
(\Cref{CaseStudyTable}). The improvement in estimator precision was weaker for
...
scores ($\Delta AIC \leq 1$; \Cref{CaseStudyTable}), and the interaction terms
were statistically significant only for the step duration of 6 hours and only
for one of the habitat covariates (\Cref{CaseStudyTable}).
...
non-linear (\Cref{DistributionParameters}). Overall, the usefulness of this
...
previous step (\Cref{PreviousDuration}). The bias in the concentration parameter
