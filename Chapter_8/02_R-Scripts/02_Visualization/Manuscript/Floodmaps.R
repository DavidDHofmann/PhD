...
# Plot
p1 <- lapply(unique(water$FloodLevel), function(i) {
  subdat <- subset(water, FloodLevel == i)
  p <- ggplot() +
    geom_raster(data = subdat, aes(x = x, y = y, fill = as.factor(Flooded))) +
    geom_sf(data = roads, col = "gray70", lwd = 0.2) +
    geom_sf(data = africa, col = "black", fill = NA, lwd = 0.3) +
    geom_sf(data = subset(oka, FloodLevel == i), col = "black", fill = NA, lty = 2) +
    geom_sf_text(data = subset(oka, FloodLevel == i), aes(label = Label), nudge_y = 0.7, nudge_x = -0.25, size = 2, parse = T, col = "red") +
...
# Add facet labels
p1[[1]] <- p1[[1]] + facet_wrap(~ "Minimum Flood")
p1[[2]] <- p1[[2]] + facet_wrap(~ "Maximum Flood")

# Remove axis in one of the plots
p1[[2]] <- p1[[2]] +rremove("y.text") + rremove("y.ticks")

# Arrange them on a single plot
p2 <- ggarrange(p1[[1]], p1[[2]]
  , nrow    = 1
  , align   = "hv"
  , labels  = "auto"
  , label.x = 0.05
