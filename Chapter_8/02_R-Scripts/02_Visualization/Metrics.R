...
# Create plot of interpatch connectivity for source area 6
p4 <- plotNet(
    pairs = rbind(
        c(1, 2)
      , c(1, 5)
...
p <- ggarrange(p1, p2, p3, p4, nrow = 4, labels = "auto")

# Store it
ggsave("04_Manuscript/99_Metrics.png"
  , plot   = p
  , bg     = "white"
  , width  = 8
  , height = 16
...
# Put the plots together
p1 <- ggarrange(p[[1]], p[[2]], p[[3]], ncol = 1, labels = c("(1)", "(2)", "(3)"))
p2 <- ggarrange(p[[4]], p[[5]], p[[6]], ncol = 1, labels = c("(4)", "(5)", "(6)"))
...
# Store the plot
ggsave("04_Manuscript/99_HumanWildlifeConflictIndividual.png"
  , plot   = p3
  , bg     = "white"
  , height = 7
  , width  = 8
...
# Put the plots together
p4 <- ggarrange(p[[1]], p[[2]], p[[3]], ncol = 1, labels = c("(1)", "(2)", "(3)"))
p5 <- ggarrange(p[[4]], p[[5]], p[[6]], ncol = 1, labels = c("(4)", "(5)", "(6)"))
...
#### Difference Maps
################################################################################
# We'll use the maximum extent
water <- subset(water, FloodLevel == "Max")

# Finally, we create difference maps for the global plots
p1 <- plotMet(subset(maps, Level == "Difference" & Metric == "Heatmap")
  , barwidth    = unit(6, "cm")
  , formula     = ~"Heatmap"
  , colorscheme = "light"
  , col_area    = "black"
...
p2 <- plotMet(subset(maps, Level == "Difference" & Metric == "Betweenness")
  , barwidth    = unit(6, "cm")
  , formula     = ~"Betweenness"
  , colorscheme = "light"
  , col_area    = "black"
...
p3 <- plotMet(subset(maps, Level == "Difference" & Metric == "HWC")
  , barwidth    = unit(6, "cm")
  , formula     = ~"Human Wildlife Conflict"
  , colorscheme = "light"
  , col_area    = "black"
  , col_country = "black"
  , col_water   = NA
...
# Remove the y-axis from the second and third plot
p2 <- p2 + rremove("y.text") + rremove("y.ticks")
p3 <- p3 + rremove("y.text") + rremove("y.ticks")

# Arrange them on a single plot
...
# Store the plot
ggsave("04_Manuscript/99_Difference.png"
  , plot   = p
  , width  = 10
  , height = 3.5
  , bg     = "white"
