...
#### Distance To Humans
################################################################################
# Description: Visualization of the distance to humans
...
plotNumberPoints <- function(data, formula = ~FloodLevel, area = unique(areas$ID), barwidth = 16, labels = F) {
  ...
  ggplot() +
    ...
    geom_sf(data = roads, col = "gray70", linewidth = 0.1) +
    geom_point(
    ...
    scale_fill_gradientn(
        # colors  = spectral(100)
        colors  = terrain.colors(100, rev = T)
...
# Apply it to create a global map
p1 <- plotNumberPoints(subset(maps, Level == "Global"), labels = T)

# I also want to generate source-area specific plots
p2 <- list()
for (i in sort(unique(maps$SourceArea))) {
  p2[[i]] <- plotNumberPoints(
      data     = subset(maps, Level == "Local" & SourceArea == i)
    , barwidth = 12
    , area     = i
  )
}
...
################################################################################
# Store the plot
ggsave("04_Manuscript/99_Distance.png"
  , plot   = p1
  , bg     = "white"
  , width  = 8
  , height = 4
...
ggsave("04_Manuscript/99_DistanceIndividual.png"
  , plot   = p5
  , bg     = "white"
  , height = 7
  , width  = 8
...
write_rds(p6, "03_Data/03_Results/DistanceDifference.rds")
