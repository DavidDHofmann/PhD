...
################################################################################
# Coordiantes at which we want to compute statistics
coords <- expand_grid(
    Date = ymd_hms(paste0(dates, "18:00:00"))
  , x    = seq(xmin, xmax, by = 10 / 111)
  , y    = seq(ymin, ymax, by = 10 / 111)
)

nightly <- pbmclapply(1:nrow(coords), mc.cores = detectCores() - 1, ignore.interactive = T, function(x) {
  pdf(file = NULL)
...
# Bind with coordinates and dates
nightly <- cbind(coords, nightly)
...
if (!file.exists("03_Data/02_CleanData/CloudCover.tif")) {
  ...
  files <- lapply(dates, function(x) {
    ...
    wei <- focalWeight(sub, d = 25 / 111, type = "circle")
    sub <- focal(sub, wei, fun = "mean")
    names(sub) <- substr(x, start = 1, stop = 10)
    setTxtProgressBar(pb, value = which(dates == x))
    return(sub)
  }) %>% do.call(c, .)
